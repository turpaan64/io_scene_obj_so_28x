...
def write_file(filepath, objects, depsgraph, scene,
    ...
    with ProgressReportSubstep(progress, 2, "OBJ Export path: %r" % filepath, "OBJ Export Finished") as subprogress1:
        with open(filepath, "w", encoding="utf8", newline="\n") as f:
            ...
            totverts = totuvco = totvcco = totno = 1
            ...
            for i, ob_main in enumerate(objects):
                ...
                for ob, ob_mat in obs:
                    with ProgressReportSubstep(subprogress1, 6) as subprogress2:
                        vc_unique_count = uv_unique_count = no_unique_count = 0
                        ...
                        if ( me.vertex_colors.active ):
                            vc_layer = me.vertex_colors.active.data[:]

                        # Vertex Colors
                        if vc_layer != None:
                            # in case removing some of these dont get defined.
                            vc = f_index = vc_index = vc_key = vc_val = vc_ls = None

                            vc_face_mapping = [None] * len(face_index_pairs)

                            vc_dict = {}
                            vc_get = vc_dict.get
                            for f, f_index in face_index_pairs:
                                vc_ls = vc_face_mapping[f_index] = []
                                for vc_index, l_index in enumerate(f.loop_indices):
                                    vc = vc_layer[l_index].color
                                    # include the vertex index in the key so we don't share UV's between vertices,
                                    # allowed by the OBJ spec but can cause issues for other importers, see: T47010.

                                    # this works too, shared UV's for all verts
                                    #~ uv_key = veckey2d(uv)
                                    vc_key = loops[l_index].vertex_index, veckey2d(vc)

                                    vc_val = vc_get(vc_key)
                                    if vc_val is None:
                                        vc_val = vc_dict[vc_key] = vc_unique_count
                                        #fw('vc{:9f}{:9f}{:9f}{:9f}\n' % (vc[0] * 255, vc[1] * 255, vc[2] * 255))
                                        if len(vc) == 4:
                                            fw("vc{:9f}{:9f}{:9f}{:9f}\n".format(vc[0], vc[1], vc[2], vc[3]))
                                        else:
                                            fw("vc{:9f}{:9f}{:9f}{:9f}\n".format(vc[0], vc[1], vc[2], 1.0))
                                        vc_unique_count += 1
                                    vc_ls.append(vc_val)

                            del vc_dict, vc, f_index, vc_index, vc_ls, vc_get, vc_key, vc_val
                            # Only need vc_unique_count and vc_face_mapping
                       # else:
                        #    for v in me_verts:
                       #         fw("vc 1.000000 1.000000 1.000000 1.000000\n")
                        ...
                        for f, f_index in face_index_pairs:
                            ...
                            if vc_layer:
                                ...
                                for vi, v, li in f_v:
                                    fw(" %d" % (totvcco + vc_face_mapping[f_index][vi]))
